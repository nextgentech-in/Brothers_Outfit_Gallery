...
const emailOtpStore = new Map(); // email -> { otp, expiresAt, attempts, createdAt }
const EMAIL_OTP_EXPIRY_MS = 5 * 60 * 1000; // 5 minutes
...
setInterval(() => {
  ...
  for (const [email, record] of emailOtpStore.entries()) {
    if (now > record.expiresAt) emailOtpStore.delete(email);
  }
}, 5 * 60 * 1000).unref();

// Send OTP to Email
app.post(['/api/email-otp/send', '/email-otp/send'], async (req, res) => {
  if (!checkSensitiveRateLimit(req, res, 10)) return;
  ...
  // Prevent spamming (minimum 30 seconds between resends)
  if (existing && (now - existing.createdAt < 30 * 1000)) {
    const waitSec = Math.ceil((30 * 1000 - (now - existing.createdAt)) / 1000);
  ...
  emailOtpStore.set(cleanEmail, {
    otp: generatedOtp,
    expiresAt: now + EMAIL_OTP_EXPIRY_MS,
    attempts: 0,
    createdAt: now
  });

  console.log(`\n======================================================`);
  console.log(`[EMAIL OTP] Email: ${cleanEmail} | OTP Code: ${generatedOtp} (Valid 5 mins)`);
  console.log(`======================================================\n`);
  ...
  try {
    if (process.env.SMTP_USER && process.env.SMTP_PASS) {
      await mailTransporter.sendMail({
        from: '"Brothers Outfit Gallery" <noreply@brothersoutfit.com>',
        to: cleanEmail,
        subject: `${generatedOtp} — Your Brothers Outfit Verification Code`,
        html: `
          <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background: #ffffff; border-radius: 12px; border: 1px solid #eee;">
            <div style="text-align: center; margin-bottom: 24px;">
              <h2 style="margin: 0; color: #0f172a; font-size: 20px; letter-spacing: 1px;">BROTHERS OUTFIT GALLERY</h2>
              <p style="color: #64748b; font-size: 13px; margin: 8px 0 0;">Email Verification</p>
            </div>
            <div style="text-align: center; padding: 24px; background: #f8fafc; border-radius: 8px; margin-bottom: 20px;">
              <p style="color: #334155; font-size: 14px; margin: 0 0 12px;">Your verification code is:</p>
              <div style="font-size: 36px; font-weight: 800; letter-spacing: 8px; color: #0f172a; font-family: monospace;">${generatedOtp}</div>
              <p style="color: #94a3b8; font-size: 12px; margin: 12px 0 0;">Valid for 5 minutes</p>
            </div>
      ...
      console.log(`[EMAIL OTP] Verification email sent to ${cleanEmail}`);
    } else {
      console.warn('[EMAIL OTP] SMTP not configured — OTP logged to console only.');
    }
  } catch (emailErr) {
    console.warn('[EMAIL OTP] Email send error:', emailErr.message);
  }

  return res.json({
    success: true,
    message: `Verification code sent to ${cleanEmail.slice(0, 3)}***@${cleanEmail.split('@')[1]}`
  });
});

// Verify Email OTP
app.post(['/api/email-otp/verify', '/email-otp/verify'], (req, res) => {
  ...
  if (!record) {
    return res.status(400).json({ error: 'No active OTP found or code has expired. Please request a new code.' });
  }
  ...
  console.log(`[EMAIL OTP SUCCESS] ${cleanEmail} verified successfully.`);
  return res.json({
    success: true,
    verified: true,
    email: cleanEmail,
