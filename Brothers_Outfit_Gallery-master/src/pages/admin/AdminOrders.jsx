...
export default function AdminOrders() {
  ...
  const executeCancelOrder = async () => {
    ...
    const finalReason = cancelReasonPreset === 'Other' 
        ...
        : `${cancelReasonPreset}${cancelReasonCustom ? ` - ${cancelReasonCustom}` : ''}`;
    ...
    try {
      ...
      await updateOrderStatus(order.id, 'Cancelled', {
        cancellationReason: finalReason || 'Cancelled by Store Admin',
  ...
  return (
    <div className="admin-orders-page">
              ...
              <tr key={o.id}>
                ...
                <td>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    ...
                    {o.status !== 'Cancelled' ? (
                      <button
                        onClick={() => handleCancelOrderClick(o)}
          ...
          <div className="order-details-modal-card" onClick={e => e.stopPropagation()}>
            ...
            <div className="order-details-modal-body">
              ...
              {selectedOrder.status === 'Cancelled' && selectedOrder.cancellationReason && (
                <div style={{ marginTop: '14px', background: '#fef2f2', border: '1px solid #fca5a5', padding: '12px', borderRadius: '8px' }}>
                  <div style={{ fontSize: '12px', color: '#991b1b', fontWeight: '800', marginBottom: '4px', textTransform: 'uppercase' }}>Cancellation Reason</div>
                  <div style={{ fontSize: '13px', color: '#7f1d1d' }}>{selectedOrder.cancellationReason}</div>
                </div>
              )}
      ...
      {/* Cancel Order Modal */}
      {cancelModalOrder && (
        <div className="tracking-modal-overlay" onClick={() => setCancelModalOrder(null)}>
          <div className="admin-cancel-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body" style={{ padding: '20px' }}>
              <p style={{ fontSize: '13.5px', color: '#334155', marginBottom: '16px', fontWeight: '500' }}>
                Please provide a reason for cancelling this order. This reason will be visible to the customer.
                {cancelModalOrder.waybill && <span style={{display: 'block', marginTop: '8px', color: '#b91c1c', fontWeight: '700'}}>⚠️ This will also cancel the Delhivery pickup schedule.</span>}
              </p>

              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#0f172a', marginBottom: '6px', textTransform: 'uppercase' }}>Select Reason</label>
                <select 
                ...
                >
                  ...
                  <option value="Unreachable / Invalid Address">Unreachable / Invalid Address</option>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#0f172a', marginBottom: '6px', textTransform: 'uppercase' }}>Custom Message (Optional)</label>
                <textarea 
                  value={cancelReasonCustom}
                  onChange={(e) => setCancelReasonCustom(e.target.value)}
                  placeholder="Additional context or custom reason..."
                  rows="3"
                  style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '14px', fontFamily: 'inherit', resize: 'vertical' }}
                />
              </div>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '24px' }}>
                <button 
                  ...
                  style={{ flex: 1, padding: '12px', background: '#f1f5f9', color: '#334155', border: 'none', borderRadius: '6px', fontWeight: '700', cursor: 'pointer' }}
                >
                  Cancel & Go Back
                </button>
                <button 
                  onClick={executeCancelOrder}
                  style={{ flex: 1, padding: '12px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '6px', fontWeight: '700', cursor: 'pointer' }}
                >
