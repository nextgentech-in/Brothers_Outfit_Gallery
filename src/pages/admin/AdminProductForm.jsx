...
import { createProduct, updateProduct, getAdminProductById, uploadProductImage, deleteProductImage, generateProductId } from '../../services/adminService';
...
export default function AdminProductForm() {
          ...
          if (data.images && data.images.length > 0) {
            const mappedImages = data.images.map((img, idx) => {
              if (typeof img === 'string') {
                return { url: img, path: null, isPrimary: data.thumbnailUrl === img || idx === 0 };
              }
              return img;
            });
  ...
  const onSelectFiles = (e) => {
    ...
    files.forEach((file) => {
      ...
      if (file.size > MAX_FILE_SIZE) {
        alert(`${file.name}: This image is larger than 2 MB. Do not upload.`);
        hasErrors = true;
        return;
      }
  ...
  const removeExistingImage = (index) => {
    setExistingImages(prev => {
      ...
      if (removed.path) {
        setImagesToDelete(d => [...d, removed.path]);
      }
      return copy;
    });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (let i = 0; i < pendingImages.length; i++) {
        ...
        const storagePath = `products/${finalProductId}/${uniqueFileName}`;
        
        const url = await uploadProductImage(item.file, storagePath);
           ...
           <div className="admin-form-actions-bottom" style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginTop: '32px' }}>
              ...
              {submitting && (
                  <p style={{fontSize:'12px', color:'#78716c', textAlign:'center', marginTop:'-8px'}}>
                     Uploading valid images directly to Firebase...
                  </p>
              )}
              <button 
